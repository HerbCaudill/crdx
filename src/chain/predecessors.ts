...
export const getCommonPredecessorHash = memoize((chain: SignatureChain<any>, a: string, b: string) => {
  if (a === b) return a
...
export const isPredecessor = <T extends Action>(chain: SignatureChain<T>, a: Link<T>, b: Link<T>): boolean => {
  return (
    a !== undefined &&
    b !== undefined &&
...
export const getPredecessors = <T extends Action>(chain: SignatureChain<T>, link: Link<T>): Link<T>[] =>
  getPredecessorHashes(chain, link.hash)
    .map(h => chain.links[h])
    .filter(link => link !== undefined)

export const getCommonPredecessor = <T extends Action = Action>(chain: SignatureChain<T>, a: Link<T>, b: Link<T>) => {
